...
# ── STC Google Sheet cache ────────────────────────────────────────────────────
# The live source of truth is the published Google Sheet on the STC witness-slip
# page: https://www.strongtownschicago.org/witness-slips
# We fetch it as CSV, cache to disk for 12 h, and fall back to STC_TRACKED_BILLS
# if the live fetch fails.
STC_SHEET_CSV_URL = (
    "https://docs.google.com/spreadsheets/d/e/"
    "2PACX-1vQPZRO3TS-AV_NoWY0qXf4W1NfUn-Q9BfCTPwZsrTS6TbWReDh3vZYHTUTWyqAZ9GWcZFakgw88j4po"
...
def _fetch_stc_sheet() -> dict:
    ...
    tracked = {}
    for row in rows:
        norm = {str(k).strip().lower(): (v or "").strip() for k, v in row.items()}
        # Try common column name variants for bill ID
        raw_id = (norm.get("bill") or norm.get("bill number") or
                  norm.get("bill #") or norm.get("bill no") or norm.get("bill id"))
        ...
        pos_raw = (norm.get("position") or norm.get("stance") or
                   norm.get("witness slip position") or "").upper()
        stance = "Opponent" if "OPPONENT" in pos_raw or "OPPOSE" in pos_raw else "Proponent"
        tracked[bill_id] = (category, description, stance)
    if not tracked:
        raise ValueError("No rows parsed from STC Google Sheet")
    return tracked
...
def load_stc_tracked_bills(force_refresh: bool = False) -> dict:
    ...
    # Try cache first
    if not force_refresh and STC_CACHE_PATH.exists():
        try:
            cached = json.loads(STC_CACHE_PATH.read_text(encoding="utf-8"))
            if now - cached.get("fetched_at", 0) < STC_CACHE_TTL and cached.get("bills"):
                bills = {k: tuple(v) for k, v in cached["bills"].items()}
                print(f"📋 Using cached STC tracked bills ({len(bills)} bills)")
                return bills
        except Exception:
    ...
    # Live fetch
    try:
        bills = _fetch_stc_sheet()
        STC_CACHE_PATH.parent.mkdir(parents=True, exist_ok=True)
        STC_CACHE_PATH.write_text(
            json.dumps({"fetched_at": now, "bills": {k: list(v) for k, v in bills.items()}},
                       indent=2, ensure_ascii=False),
            encoding="utf-8"
        )
        print(f"✅ Loaded {len(bills)} STC tracked bills from Google Sheet")
        return bills
    except Exception as e:
        print(f"⚠️  Could not fetch STC sheet: {e}")

    # Try stale cache
    if STC_CACHE_PATH.exists():
        try:
            cached = json.loads(STC_CACHE_PATH.read_text(encoding="utf-8"))
            if cached.get("bills"):
                bills = {k: tuple(v) for k, v in cached["bills"].items()}
                print(f"⚠️  Using stale cache ({len(bills)} bills)")
                return bills
        except Exception:
            pass

    # Hard-coded fallback
    print("⚠️  Falling back to hard-coded STC tracked bills")
    return STC_TRACKED_BILLS
...
class OpenStatesParser:
    ...
    def parse_data_directory(data_dir: str) -> List[Bill]:
        """Parse the govbot-openstates-scrapers/il-legislation flat JSON format.

        The repo stores all bills as flat files under _data/il/bill_*.json.
        Each file is one OpenStates bill record with this shape:

          {
            "identifier": "HB2454",
            "title": "BICYCLES-ROADWAYS",
            "subject": ["Transportation"],
            "from_organization": "~{\"classification\": \"lower\"}",
            "classification": ["bill"],
            "sponsorships": [{"name": "...", "primary": true, ...}],
            "actions": [{"description": "...", "classification": [...]}],
            "sources": [{"url": "https://ilga.gov/Legislation/BillStatus?..."}],
            ...
          }

        Some files in the same directory are vote-event records (they lack
        a top-level string identifier matching HB/SB); those are skipped.
        Pass data_dir pointing to _data/il/ inside the cloned repo.
        """
    ...
    def scrape_ilga_bill_hearings() -> dict:
        ...
        detail_re  = re.compile(r'hearings/details/(\d+)/(\d+)', re.I)
        cmt_id_re  = re.compile(r'Committees/Members/(\d+)', re.I)

        today    = datetime.now().strftime('%Y-%m-%d')
        end_date = (datetime.now() + timedelta(days=60)).strftime('%Y-%m-%d')

        chambers = [
            ('Senate', 'https://ilga.gov/Senate/Committees', 1),
            ('House',  'https://ilga.gov/House/Committees',  2),
        ]

        seen_detail_urls: set = set()
        detail_entries: list  = []  # (chamber_name, committee_id, hearing_id)

        # ── Step 1 & 2: collect committee IDs from index pages ────────────
        for chamber_name, index_url, chamber_api_id in chambers:
            try:
            ...
            committee_ids = sorted(set(cmt_id_re.findall(resp.text)))
            print(f'   {chamber_name}: found {len(committee_ids)} committee IDs from index')

            # ── Step 3: per-committee hearing pages ───────────────────────
            for cid in committee_ids:
                hearings_url = (
                    f'https://ilga.gov/{chamber_name}/Committees/Hearings/{cid}'
                )
                try:
                    cr = requests.get(hearings_url, timeout=20, headers=headers)
                    cr.raise_for_status()
                except Exception:
                    continue

                for m in detail_re.finditer(cr.text):
                    cid2, hid = m.group(1), m.group(2)
                    key = (chamber_name, cid2, hid)
                    url = (
                        f'https://ilga.gov/{chamber_name}/hearings/details/{cid2}/{hid}'
                    )
                    if url not in seen_detail_urls:
                        seen_detail_urls.add(url)
                        detail_entries.append(key)

        # ── Step 4: supplement from the date-range API ───────────────────
        for chamber_name, _, chamber_api_id in chambers:
            api_url = (
                f'https://ilga.gov/API/Hearings/GetHearingsListByRange'
                f'?ChamberId={chamber_api_id}&GaId=18'
                f'&BeginDate={today}&EndDate={end_date}'
            )
            try:
                ar = requests.get(api_url, timeout=20, headers=headers)
                ar.raise_for_status()
                api_hearings = ar.json() if ar.text.strip().startswith('[') else []
            except Exception:
                api_hearings = []

            for h in api_hearings if isinstance(api_hearings, list) else []:
                hid = str(h.get('HearingId') or h.get('hearingId') or h.get('id') or '')
                cid = str(h.get('CommitteeId') or h.get('committeeId') or '')
                if not hid:
                    continue
                url = (
                    f'https://ilga.gov/{chamber_name}/hearings/details/{cid}/{hid}'
                    if cid else
                    f'https://ilga.gov/{chamber_name}/hearings/details/{hid}'
                )
                if url not in seen_detail_urls:
                    seen_detail_urls.add(url)
                    detail_entries.append((chamber_name, cid, hid))

        print(f'   Discovered {len(detail_entries)} hearing detail pages')

        # ── Step 5: fetch each detail page ───────────────────────────────
        now = datetime.now()
        for chamber_name, cid, hid in detail_entries:
            detail_url = (
                f'https://ilga.gov/{chamber_name}/hearings/details/{cid}/{hid}'
                if cid else
                f'https://ilga.gov/{chamber_name}/hearings/details/{hid}'
            )
            slip_url = detail_url + '/createwitnessslip'
            ...
            # Parse date — ILGA format: "4/23/2026 1:30 PM"
            dt = None
            us_m = re.search(
                r'Date:\s*</span>\s*([\d]{1,2}/[\d]{1,2}/[\d]{4})\s+([\d]{1,2}:[\d]{2}\s*[AP]M)',
                detail_text, re.I,
            )
            if not us_m:
                # looser fallback
            ...
            if us_m:
                date_str = us_m.group(1)
                time_str = us_m.group(2).replace(' ', '')
                try:
                    dt = datetime.strptime(f'{date_str} {time_str}', '%m/%d/%Y %I:%M%p')
                except ValueError:
                    pass
            if dt is None:
                continue  # skip pages with no parseable date

            # Only keep future (or today's) hearings
            if dt.date() < now.date():
                continue
            ...
            for bill_match in bill_re.finditer(detail_text):
                ...
                bills_found.append(bill_id)

            if bills_found:
                print(
                    f'   🧧 {detail_url}\n'
                    f'       Date: {dt.strftime("%b %-d %Y %-I:%M %p")}\n'
                    f'       Bills: {list(dict.fromkeys(bills_found))}'
                )
...
def main():
    ...
    import re as _re
    stc_bills = load_stc_tracked_bills()
    feed_ids = {b.bill_number for b in bills}
    stc_added = 0
    for bill_num, (category, stc_desc, stance) in stc_bills.items():
    ...
    for b in bills:
        if not hasattr(b, 'stance'):
            info = stc_bills.get(b.bill_number)
    ...
    if args.feed:
        ...
        # data-dir mode: all topic-tagged bills are actionable (same as feed mode).
        # STC stubs always have a subject, so every tracked bill is included.
        # Hearing data is enriched onto matching bills but never used as a filter.
        actionable = [b for b in bills if b.subjects]
    ...
    json_output = [
        {'bill_number': b.bill_number,
         ...
         'source': 'stc_tracked' if b.bill_number in {re.sub(r'\s+','',k.upper()) for k in stc_bills} else 'feed',
        }
        for b in actionable
    ]
